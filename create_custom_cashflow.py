...
add_expr('balance', 'aggregation',
    'CF_NI.balance + CF_DEP.balance + CF_FX.balance + CF_REC.balance + CF_UBR.balance + CF_PRE.balance + CF_OCA.balance + CF_AP.balance + CF_CC.balance + CF_DR.balance + CF_OCL.balance + CF_ACR.balance', top_id)
...
add_expr('balance', 'domain', "[('account_id.account_type', '=', 'liability_non_current')]", ncl_id)
...
for (expr_id, label, engine, formula, line_id) in exprs:
    ...
    INSERT INTO account_report_expression
        (id, report_line_id, label, engine, formula, date_scope,
         create_uid, create_date, write_uid, write_date)
    VALUES (%s, %s, %s, %s, %s, 'strict_range',
            1, NOW(), 1, NOW())
